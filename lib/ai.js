import { OPENAI_MODEL, GROQ_MODEL, BRIEFING_ITEMS_PER_SECTION } from './config.js';
...
const SHARED_RULES = [
  'MANDATÓRIO: TODO o conteúdo de texto (title, summary, context, whyItMatters) DEVE ser escrito em Português do Brasil. NUNCA DEIXE EM INGLÊS.',
  'Use apenas os itens fornecidos, sem inventar fatos.',
  `Devolva no MÁXIMO ${BRIEFING_ITEMS_PER_SECTION} tópicos para CADA seção (Brasil, Mundo, Economia, Tecnologia). É estritamente proibido exceder esse limite.`,
  'A manchete do dia deve ter titulo e resumo curto de 2 a 3 linhas.',
  'Cada contexto deve ser extremamente curto, editorial e direto ao ponto, com no máximo 140 caracteres.',
  ...
  'Por que isso importa deve ter 3 bullets praticos, especificos e conectados as noticias do dia.',
  'Preserve fonte e link de cada item.'
];

const SYSTEM_PROMPT = 'Você é um editor sênior de briefing jornalístico no Brasil. O IDIOMA DE SAÍDA DEVE SER ESTRITAMENTE PORTUGUÊS DO BRASIL. É TERMINANTEMENTE PROIBIDO DEIXAR TEXTO EM INGLÊS. Seja conciso, rigoroso e útil. Retorne somente JSON válido.';
