...
import { enrichWithGroq, enrichWithOpenAI, buildSpecificWhyItMatters } from './ai.js';
...
export async function generateBriefing(date = todayISO(), force = false) {
  ...
  let briefing;
  try {
    // Tenta Groq primeiro (mais rápido e gratuito); fallback para OpenAI
    briefing = await enrichWithGroq(date, collected) || await enrichWithOpenAI(date, collected);
  } catch (error) {
    console.error('Erro na geração via IA:', error.message);
