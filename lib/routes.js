...
export async function routeApi(req, res, pathname) {
  ...
  if (req.method === 'GET' && pathname === '/api/health') {
    const provider = process.env.GROQ_API_KEY ? 'groq' : process.env.GEMINI_API_KEY ? 'gemini' : process.env.OPENAI_API_KEY ? 'openai' : 'rss';
    const model = provider === 'groq' ? GROQ_MODEL : provider === 'gemini' ? process.env.GEMINI_MODEL || 'gemini-1.5-flash' : provider === 'openai' ? OPENAI_MODEL : null;
