...
function splitMessage(fullMessage, maxLen) {
  ...
  // Removido a adicao de tags (ex: [1/5], continua...) a pedido do usuario
  // para deixar a mensagem mais limpa.
...
export async function sendWhatsApp(briefing) {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    // Espera entre partes para não sobrecarregar o gateway (5s para segurança extra contra rate limit)
    if (i < parts.length - 1) {
